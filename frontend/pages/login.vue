...
<template>
  <div class="flex min-h-screen items-center justify-center px-4">
    <form class="w-full max-w-sm space-y-4 rounded-xl border border-stone-200 bg-white p-8 shadow-sm" @submit.prevent="onSubmit">
      ...
      <p class="text-sm text-stone-500">Sign in to your vault.</p>
      ...
      <div class="space-y-1">
        ...
        <input id="password" v-model="password" type="password" required class="w-full rounded-md border border-stone-300 px-3 py-2 outline-none focus:border-espresso" />
      </div>
      ...
      <p class="text-center text-sm text-stone-500">
        No account yet?
        <NuxtLink to="/register" class="font-medium text-espresso underline">Register</NuxtLink>
      </p>
    </form>
  </div>
</template>
